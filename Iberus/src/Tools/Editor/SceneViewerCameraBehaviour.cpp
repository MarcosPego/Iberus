...
namespace Iberus {
	...
	SceneViewerCameraBehaviour::SceneViewerCameraBehaviour()
		: keyBinds(CreateDefaultKeyBinds()) {
		type = "b_scene_viewer_camera";
	}
	...
	void SceneViewerCameraBehaviour::Update(EntityId entity, World& world, double deltaTime) {
		...
		Vec3 forwardXY(sinYaw, cosYaw, 0.0f);
		Vec3 rightXY(cosYaw, -sinYaw, 0.0f);

		Vec3 movement(0, 0, 0);
		if (IsKeyBindPressed(keyBinds[SceneViewerAction::MoveForward])) {
			movement += forwardXY;
		}
		if (IsKeyBindPressed(keyBinds[SceneViewerAction::MoveBackward])) {
			movement -= forwardXY;
		}
		if (IsKeyBindPressed(keyBinds[SceneViewerAction::MoveLeft])) {
			movement -= rightXY;
		}
		if (IsKeyBindPressed(keyBinds[SceneViewerAction::MoveRight])) {
			movement += rightXY;
		}
		if (IsKeyBindPressed(keyBinds[SceneViewerAction::MoveUp])) {
			movement.z += 1.0f;
		}
		if (IsKeyBindPressed(keyBinds[SceneViewerAction::MoveDown])) {
			movement.z -= 1.0f;
		}
